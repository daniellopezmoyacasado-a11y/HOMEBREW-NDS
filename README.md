...
The project uses devkitpro & devkitARM to compile C++ code into an .nds file, which can be directly uploaded to a hacked nintendo ds cartridge (the ones that had a microSD). I had a M3 DS REAL Cartridge from the early 2000s that runs on M3 Sakura. M3 Sakura, just like the DeSmuMe emulator, doesn't run these custom roms. It gets stuck and shows a blank white screen.
